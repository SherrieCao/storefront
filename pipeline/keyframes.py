...
def run_keyframes(run: Run, brief: dict[str, Any], inventory: dict[str, Any],
    ...
    def do_keyframe(seg: dict[str, Any]) -> None:
        ...
        if seg["type"] == "seedance_shot":
            ...
            # before/after (D43/D44): a `before` photo is the raw PROBLEM-STATE image — show it PLAIN, not a
...
def _moodboard_prompt(seg: dict[str, Any], mood: str) -> str:
    # Reverted to the original inventive board (D57, undoing D53/D55/D56): the model is NOT restricted to
    # only the provided photos — it composes the real photos as cut-outs AND may generate complementary
    # imagery that fits the board's mood, like a real art-directed scrapbook/pinboard. (The enhance black-
    # frame guard, D54, stays — it's a bug fix, not a restriction — so the real photos still arrive intact.)
