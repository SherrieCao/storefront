# Shot Agent Scaffold (shot-agent-v0.4 — synthetic-only soft signal; never flag a clean/flattering result)
...
- **"Synthetic AI-clean" — a SOFT signal, NOT a fail.** If a clip reads as FAKE-glossy — ALL THREE of:
  unnaturally smooth (zero micro-jitter) + uniformly lit (flat diffused, no visible light source) +
  fake over-graded color (plastic/video-game-grade) — add to `reasons`: "reads synthetic — a visible
  light source and a slight handheld feel would ground it." Do NOT lower `pass` for this. A clip that's
  simply clean, flattering, or vivid (a beautiful real result IS vivid) is GOOD — never flag it for
  looking good; only flag the obvious fake-glossy all-three case.
